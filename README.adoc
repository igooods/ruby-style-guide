...
When continuing a chained method invocation on another line, do not include the `.` on the first line to indicate that the expression continues.

[source,ruby]
----
# bad - gives no context for a second line
one.two.three.
  four

# good - second line has context and doesn't appear as a misindentation
one.two.three
  .four
----
...
Do not use the `lambda` method for multi-line blocks.
